...
export class EuropeanACPowerFlowSimulator {
  ...
  private buildLineData(): ACLineData[] {
    ...
    for (const line of this.lines.values()) {
      ...
      // Get base voltage for per-unit conversion
      const baseVoltage = this.getLineVoltage(line);
      const baseImpedance = (baseVoltage * baseVoltage) / this.baseMVA;
  ...
  private getLineVoltage(line: PowerLine): number {
    // Get voltage level from connected components
    const fromComponent = this.components.get(line.from);
    const toComponent = this.components.get(line.to);
    
    if (fromComponent) return fromComponent.voltage;
    if (toComponent) return toComponent.voltage;
    return 400; // Default to 400kV
  }
  ...
  private calculatePowerFlows(buses: ACBusData[], lines: ACLineData[], voltages: { magnitude: number, angle: number }[]): { [lineId: string]: { active: number, reactive: number, apparent: number } } {
    ...
    for (const line of lines) {
      ...
      powerFlows[line.id] = {
        active: activePower * this.baseMVA, // Convert back to MW
        reactive: reactivePower * this.baseMVA, // Convert back to MVAr
        apparent: apparentPower * this.baseMVA // Convert back to MVA
      };
    }
